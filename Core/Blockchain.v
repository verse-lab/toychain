...
case: Geq=>[Eq|Gt].
- (* Eq is false due to mint_ext and Nb *) admit.
- suff X: btChain (btExtend bt b) = compute_chain (btExtend cbt b) b.
  have Y: (btChain (btExtend bt b) > [:: GenesisBlock]).
    admit.
  by rewrite -X; apply (lesser_elim Y Gt).
Qed.
