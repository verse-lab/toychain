...
Module Type BLOCKCHAIN_PROTOCOL (Import BC : BLOCKCHAIN) (Import BCF : BLOCKCHAIN_FACTS BC).
...
Axiom eq_msg_typeP : Equality.axiom eq_msg_type.
...
Axiom eq_msgP : Equality.axiom eq_msg.
...
Axiom peers_uniq_init : forall (n : Address), uniq [::n].
...
(* Proofs *)
Axiom procMsg_id_constant : forall (s1 : State) from (m : Message) (ts : Timestamp),
    id s1 = id (procMsg s1 from m ts).1.

Axiom procInt_id_constant : forall (s1 : State) (t : InternalTransition) (ts : Timestamp),
...
Axiom procMsg_non_block_nc_blockTree :
  forall (s1 : State) from (m : Message) (ts : Timestamp),
    let: s2 := (procMsg s1 from m ts).1 in
    (forall b, m != BlockMsg b) ->
    blockTree s1 = blockTree s2.

Axiom procMsg_non_block_nc_btChain :
  forall (s1 : State) from (m : Message) (ts : Timestamp),
    let: s2 := (procMsg s1 from m ts).1 in
    (forall b, m != BlockMsg b) ->
    btChain (blockTree s1) = btChain (blockTree s2).

Axiom procMsg_known_block_nc_blockTree :
...
End BLOCKCHAIN_PROTOCOL.

Module BlockChainProtocol (Import BC : BLOCKCHAIN) (Import BCF : BLOCKCHAIN_FACTS BC) : BLOCKCHAIN_PROTOCOL BC BCF.
